...
# Inserted styling code manually
# StreamSub App Styling - Match Website Design
...
class StreamSubGUI:
    def __init__(self):
        ...
        self.selected_lang = tk.StringVar(value="🇪🇸 Spanish")
        self.is_recording = False
        self.overlay_visible = True
        
    def setup_window(self):
        """Configure main window to match website aesthetic"""
        self.root.title("StreamSub - Live Subtitle Generator")
    ...
    def setup_styles(self):
        """Configure ttk styles to match website theme"""
        self.style = ttk.Style()
        self.style.theme_use('clam')
        
        # Primary Button (like website's main CTA buttons)
    ...
    def create_card_frame(self, parent, **kwargs):
        """Create a card-like frame matching website design"""
    ...
    def create_main_interface(self):
        """Create the main interface matching website layout"""
    ...
    def create_header(self, parent):
        """Create header section like website hero"""
    ...
    def create_language_section(self, parent):
        """Language selection card"""
        lang_card = self.create_card_frame(parent)
        lang_card.pack(fill='x', pady=(0, 20))
        
        # Card header
        header_frame = tk.Frame(lang_card, bg=Colors.CARD_BG)
        ...
        tk.Label(header_frame,
                text="Choose the language for subtitle generation",
                bg=Colors.CARD_BG,
                fg=Colors.TEXT_SECONDARY,
                font=('Segoe UI', 10)).pack(anchor='w', pady=(4, 0))
        
        # Language dropdown
        dropdown_frame = tk.Frame(lang_card, bg=Colors.CARD_BG)
        dropdown_frame.pack(fill='x', padx=20, pady=(0, 20))
        
        self.lang_combo = ttk.Combobox(dropdown_frame,
    ...
    def create_controls_section(self, parent):
        """Control buttons card"""
        controls_card = self.create_card_frame(parent)
        controls_card.pack(fill='x', pady=(0, 20))
        
        # Card header
        header_frame = tk.Frame(controls_card, bg=Colors.CARD_BG)
        header_frame.pack(fill='x', padx=20, pady=(20, 10))
        ...
        # Buttons frame
        buttons_frame = tk.Frame(controls_card, bg=Colors.CARD_BG)
        buttons_frame.pack(fill='x', padx=20, pady=(10, 20))
        
        # Start/Stop button (dynamic)
        self.control_btn = ttk.Button(buttons_frame,
                                     text="🎙️ Start Recording",
                                     style='Success.TButton',
                                     command=self.toggle_recording)
        self.control_btn.pack(fill='x', pady=(0, 10))
        ...
        self.overlay_btn = ttk.Button(buttons_frame,
                                     text="👁️ Hide Overlay",
                                     style='Secondary.TButton',
    ...
    def create_status_section(self, parent):
        ...
        # Card header
        header_frame = tk.Frame(status_card, bg=Colors.CARD_BG)
        header_frame.pack(fill='x', padx=20, pady=(20, 10))
        ...
        # Stats grid
        stats_frame = tk.Frame(status_card, bg=Colors.CARD_BG)
        stats_frame.pack(fill='x', padx=20, pady=(0, 20))
        
        # Create stat items
        self.create_stat_item(stats_frame, "Status:", "Ready", 0)
        self.create_stat_item(stats_frame, "Words:", "0", 1)
        self.create_stat_item(stats_frame, "Characters:", "0", 2)
        self.create_stat_item(stats_frame, "Duration:", "00:00", 3)
    ...
    def create_stat_item(self, parent, label, value, row):
        ...
        tk.Label(parent,
                ...
                bg=Colors.CARD_BG,
                fg=Colors.TEXT_SECONDARY,
                font=('Segoe UI', 10)).grid(row=row, column=0, sticky='w', pady=2)
        ...
        if label == "Status:":
            ...
            self.words_label = value_label
        elif label == "Characters:":
            self.chars_label = value_label
        elif label == "Duration:":
    ...
    def create_settings_section(self, parent):
        ...
        settings_card = self.create_card_frame(parent)
        settings_card.pack(fill='x')
        
        # Card header
        header_frame = tk.Frame(settings_card, bg=Colors.CARD_BG)
        header_frame.pack(fill='x', padx=20, pady=(20, 10))
        ...
        # Quick settings
        quick_frame = tk.Frame(settings_card, bg=Colors.CARD_BG)
        quick_frame.pack(fill='x', padx=20, pady=(0, 20))
        
        # Audio device info
        tk.Label(quick_frame,
                text="🎵 Audio Input: Default Microphone",
                bg=Colors.CARD_BG,
                fg=Colors.TEXT_SECONDARY,
                font=('Segoe UI', 10)).pack(anchor='w', pady=2)
        
        tk.Label(quick_frame,
                text="🎯 Output Format: WAV (32kHz)",
                bg=Colors.CARD_BG,
                fg=Colors.TEXT_SECONDARY,
                font=('Segoe UI', 10)).pack(anchor='w', pady=2)
    ...
    def toggle_recording(self):
        ...
        if self.is_recording:
            self.control_btn.configure(text="⏹️ Stop Recording", style='Error.TButton')
            self.status_label.configure(text="Recording", fg=Colors.SUCCESS)
            # Start your recording logic here
        else:
            self.control_btn.configure(text="🎙️ Start Recording", style='Success.TButton')
    ...
    def toggle_overlay(self):
        ...
        if self.overlay_visible:
            self.overlay_btn.configure(text="👁️ Hide Overlay")
        else:
            self.overlay_btn.configure(text="👁️‍🗨️ Show Overlay")
            
    def open_settings(self):
        """Open detailed settings window"""
        self.create_settings_window()
        
    def create_settings_window(self):
        """Create comprehensive settings window"""
        settings_window = tk.Toplevel(self.root)
        settings_window.title("StreamSub Settings")
        ...
        # Audio Settings Card
        self.create_audio_settings_card(main_frame)
        
        # Overlay Settings Card  
        self.create_overlay_settings_card(main_frame)
        
        # Advanced Settings Card
        self.create_advanced_settings_card(main_frame)
        
        # Action buttons
        self.create_settings_buttons(main_frame, settings_window)
        
    def create_audio_settings_card(self, parent):
        """Audio settings section"""
        audio_card = self.create_card_frame(parent)
        audio_card.pack(fill='x', pady=(0, 16))
        ...
        content_frame = tk.Frame(audio_card, bg=Colors.CARD_BG)
        ...
        rate_combo = ttk.Combobox(content_frame,
                                 values=["16000 Hz", "32000 Hz", "44100 Hz", "48000 Hz"],
                                 state='readonly',
                                 style='Custom.TCombobox',
                                 width=15)
        rate_combo.set("32000 Hz")
        rate_combo.grid(row=0, column=1, sticky='w', padx=(10, 0), pady=5)
        ...
        channels_combo = ttk.Combobox(content_frame,
                                     values=["Mono", "Stereo"],
                                     state='readonly',
                                     style='Custom.TCombobox',
                                     width=15)
        channels_combo.set("Mono")
        channels_combo.grid(row=1, column=1, sticky='w', padx=(10, 0), pady=5)
        
    def create_overlay_settings_card(self, parent):
        """Overlay settings section"""
        overlay_card = self.create_card_frame(parent)
        overlay_card.pack(fill='x', pady=(0, 16))
        
        # Header
        tk.Label(overlay_card,
                text="👁️ Overlay Settings",
                bg=Colors.CARD_BG,
                fg=Colors.TEXT_PRIMARY,
                font=('Segoe UI', 14, 'bold')).pack(anchor='w', padx=20, pady=(20, 10))
        
        content_frame = tk.Frame(overlay_card, bg=Colors.CARD_BG)
        content_frame.pack(fill='x', padx=20, pady=(0, 20))
        ...
        font_scale = tk.Scale(content_frame,
                             from_=12, to=48,
                             orient='horizontal',
                             bg=Colors.CARD_BG,
                             fg=Colors.TEXT_PRIMARY,
        ...
        font_scale.set(24)
        font_scale.grid(row=0, column=1, sticky='ew', padx=(10, 0), pady=5)
        ...
        pos_combo = ttk.Combobox(content_frame,
                                values=["Top", "Center", "Bottom"],
                                state='readonly',
                                style='Custom.TCombobox',
                                width=15)
        pos_combo.set("Bottom")
        pos_combo.grid(row=1, column=1, sticky='w', padx=(10, 0), pady=5)
        
        content_frame.grid_columnconfigure(1, weight=1)
        
    def create_advanced_settings_card(self, parent):
        """Advanced settings section"""
        advanced_card = self.create_card_frame(parent)
        advanced_card.pack(fill='x', pady=(0, 16))
        
        # Header
        tk.Label(advanced_card,
                text="⚙️ Advanced Settings",
                bg=Colors.CARD_BG,
                fg=Colors.TEXT_PRIMARY,
                font=('Segoe UI', 14, 'bold')).pack(anchor='w', padx=20, pady=(20, 10))
        
        content_frame = tk.Frame(advanced_card, bg=Colors.CARD_BG)
        content_frame.pack(fill='x', padx=20, pady=(0, 20))
        
        # Checkboxes with website-style
        self.auto_start_var = tk.BooleanVar(value=False)
        auto_start_cb = tk.Checkbutton(content_frame,
                                      ...
                                      font=('Segoe UI', 10))
        auto_start_cb.pack(anchor='w', pady=2)
        
        self.save_log_var = tk.BooleanVar(value=True)
        save_log_cb = tk.Checkbutton(content_frame,
                                    text="Save transcription logs",
                                    ...
                                    activeforeground=Colors.TEXT_PRIMARY,
                                    font=('Segoe UI', 10))
        save_log_cb.pack(anchor='w', pady=2)
        
    def create_settings_buttons(self, parent, window):
